...
GPIO_PWM = 18 # PWM pin                             
GPIO_IN1 = 00 # motor control pin                   #has to be changed
GPIO_IN2 = 00 # motor control pin                   #has to be changed
...
def drive(ahead = True, frequency = 50000, duty_cycle = 10):
    ...
    if ahead == True:
        ...
        pi.write(GPIO_IN1, 0)                       #has to be changed
        pi.write(GPIO_IN2, 1)                       #has to be changed

    elif ahead == False:
        pi.set_mode(GPIO_IN1, pigpio.OUTPUT)
        pi.set_mode(GPIO_IN2, pigpio.OUTPUT)
        pi.write(GPIO_IN1, 1)                       #has to be changed
        pi.write(GPIO_IN2, 0)                       #has to be changed
